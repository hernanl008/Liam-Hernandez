...
--
-- Row 2 is drawn by this project, not by the pack. The pack's row 2 was
-- assumed to be a side profile but on inspection was a second
-- FRONT-facing pose — both eyes visible, face square to the camera
-- (measured: row 2 differed from row 0 by less than either differed
-- from its own mirror). So the sheet contained no left- or right-facing
-- art at all, which is why walking sideways looked like moonwalking
-- whichever way the mirror was bound: both directions were drawing the
-- same forward-facing sprite. tools/make_player_side_rows.py draws a
-- real profile over row 2 in the pack's own palette and proportions.
--
-- That generated profile faces RIGHT, and Roblox cannot mirror an
-- ImageLabel at runtime (a negative Size renders nothing rather than
-- flipping — that was tried first). So walking left is served by
...
-- for both directions, which moonwalks going left but never breaks.
--
...
local function applySprite(character: Model)
	...
	conn = RunService.Heartbeat:Connect(function(dt: number)
		...
		-- The side row faces RIGHT, so the mirror serves LEFT. This is
		-- now a property of art this project generates itself
		-- (tools/make_player_side_rows.py draws the profile facing right)
		-- rather than something inferred about a third-party sheet.
